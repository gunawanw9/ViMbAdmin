...
{
    $( "a[id|='delete-canonical']" ).bind( 'click', deleteCanonical );
    ...
    oDataTable = $( '#list_table' ).dataTable({
        'fnDrawCallback': function() {
            ...
            {if isset($options.defaults.server_side.pagination.enable) && $options.defaults.server_side.pagination.enable }
                ...
                $( "a[id|='delete-canonical']" ).unbind().bind( 'click', deleteCanonical );
...
function toggleActive( elid, id ){
    ossToggle( $( '#' + elid ), "{genUrl controller='canonical' action='ajax-toggle-active'}", { "caid": id } );
};



function deleteCanonical( event ){
    ...
    if( $.trim( $( event.target ).val() ).length >= str_len ){ 
        timeOut = setTimeout( function(){ 
            ...
            setTimeout( function(){
                ...
                $.ajax({
                  async: false,
                  url: "{genUrl controller='canonical' action='list-search' ima=$ima}/search/" + $.trim( $( event.target ).val() ),
                    ...
                    if( data !== "ko" && data.substr( 0, 1 ) == "[" )
                    {
                        ...
                        $.each( data, function( index, row ){
                               oDataTable.fnAddData([
                                    ...
                                    formatSender( row.id, row.sender ),
...
function formatSender( id, sender )
{
    var str = '<div id="canonical-sender-' + id + '" ';
    if( sender.length  > 50 )
    {
        str += 'class="have-tooltip-long" title="' + sender.replace( /[,]/g, ", ") + '"';
        sender = sender.substr( 0, 50 ) + '...'; 
    }
    str += '>' + sender + '</div>';
    return str;
}
...
function formatControlls( id )
{
    ...
    var str = '<div class="btn-group">\
            <a class="btn btn-mini have-tooltip" id="edit_canonical_' + id + '" title="Edit" href="{genUrl controller="canonical" action="edit"}/caid/' + id + '">\
    ...
    str += '<a class="btn btn-mini have-tooltip" id="delete-canonical-' + id + '" title="Delete" href="{genUrl controller="canonical" action="delete"}/caid/' + id + '">\
